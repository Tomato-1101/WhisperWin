...
- **🕘 入力履歴**: 音声入力をローカルに保存し、ホーム画面やサイドノッチから再コピーできます（Mac＝直近 200 件・貼り付け先アプリ／日時つき、Windows＝直近 10 件・日時つき）。上限を超えると古いものから自動で消えます（統計は履歴と独立した累計のため、履歴が消えても減りません）
- **☁️ Mac ⇄ Windows 履歴共有（任意・既定オフ）**: 両 OS の設定から有効化すると、Cloudflare Workers + D1 経由で音声入力履歴を共有します（自分専用・共有トークン 1 本・オフライン中はキューに溜めて自動再送）。他端末の行は Mac で `[Windows]`、Windows で `[Mac]` と表示します。詳細は `docs/HISTORY_SYNC.md`
...
- **🤖 Google Meet 議事録ボット（Mac・macOS 26 以降・自分用ビルドのみ・Chrome が必要）**: 会議の URL を渡すと、
  **ボットが裏で会議に入って文字起こしを議事録として保存**します。メニューバーの「Meet 議事録ボット」→「会議に参加…」。
  ...
  - **使い方（初回）**: ① メニューバー → 「Meet 議事録ボット」 → 「ボット用ブラウザで Google にログイン…」で
    開いた Chrome にログインし、閉じる。② 会議の URL（`https://meet.google.com/xxx-yyyy-zzz`）をコピーして
    「会議に参加…」→「参加」。③ 自分が主催者でない会議では、主催者が「voicekey 議事録ボット」の参加を
    承認する。④ 記録はメニューの状態行（「記録中 — 会議名」）で分かり、終わったら「会議から退出」。
    ログインは 1 回だけで、以後は Chrome のプロファイルに残ります（未ログインだと Meet が会議ページから
    紹介サイトへ飛ばすので参加できません。その場合はメニューにログインの案内が出ます）
...
- **履歴同期（Mac / Windows）**: 相手の端末と履歴を共有する（Cloudflare Workers + D1 経由・既定オフ・共有トークン 1 本）。詳細は `docs/HISTORY_SYNC.md`
...
#### Mac で Windows と履歴を共有する

1. 設定 → 一般 → 履歴の「Windows と履歴を共有」をオンにします。
2. 「同期サーバー URL」に `https://voicekey-history-sync.<subdomain>.workers.dev` を入力します。
3. Windows の `%LOCALAPPDATA%\voicekey\sync_token.txt` を開き、中身を「共有トークン」へ貼り付けて「保存」を押します。
4. 「送信待ち 0 件」と「最終同期 hh:mm」が表示されれば同期中です。トークンが無効な場合は、正しい値を保存し直すと再開します。

URL とトークンの実値はリポジトリへ記録しません。同期はログイン状態に依存せず、無効時は従来どおりローカル履歴だけを表示します。
...
│   └── utils/logger.py
├── sync-worker/                  # Cloudflare Worker（履歴同期・Workers + D1）
├── docs/
